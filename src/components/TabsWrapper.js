    ...
    <Tabs
      ...
      <TabsList className='flex max-md:flex-wrap h-auto gap-3 md:gap-5 w-full'>
        {tabsItems.map((item, index) => (
          <TabsTrigger
            className={`grow border-black border-[1px] data-[state=active]:bg-[#4F16F2] data-[state=active]:text-white text-black rounded-none tracking-normal max-md:px-1 md:mb-5 px-4 grow max-w-[calc(100% / ${tabsItems.length})]`}
            key={index}
            value={item.value}>
            {item.label}
          </TabsTrigger>
